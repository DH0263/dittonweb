...
function KioskInternal() {
  const [status, setStatus] = useState('idle'); // idle, scanning, success, error
  const [message, setMessage] = useState('');
  const [studentInfo, setStudentInfo] = useState(null);
  const [actionType, setActionType] = useState('outing'); // outing or return
  ...
  const handleScan = async (type) => {
    ...
    try {
      ...
      if (type === 'outing') {
        ...
        successMessage = `${student.name}님, 외출 처리되었습니다`;
      } else {
        // 복귀 - 기존 외출 기록 찾아서 end_time 업데이트 필요
        // 간단하게 새로운 기록으로 처리
        outingData = {
          student_id: userId,
          date: today,
        ...
        successMessage = `${student.name}님, 복귀 완료!`;
      }

      const outingResponse = await fetch(`${DITTONWEB_API}/outings/`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (!outingResponse.ok) {
        const errorData = await outingResponse.json().catch(() => ({}));
        throw new Error('기록 실패: ' + (errorData.detail || '알 수 없는 오류'));
      }
        ...
        <div style={{
          ...
          <button
            ...
            style={{
              ...
              fontWeight: 'bold',
              color: 'white',
              backgroundColor: status === 'scanning' ? '#9ca3af' : '#f59e0b',
              border: 'none',
              borderRadius: '16px',
          ...
          <button
            ...
            style={{
              ...
              fontWeight: 'bold',
              color: 'white',
              backgroundColor: status === 'scanning' ? '#9ca3af' : '#3b82f6',
              border: 'none',
              borderRadius: '16px',
              cursor: status === 'scanning' ? 'not-allowed' : 'pointer',
              transition: 'all 0.3s',
